<template>
  <div class="admin-page">
    <div class="container">
      ...
      <!-- Episodes -->
      <section v-if="tab === 'episodes'" class="tab-panel">
        <h3 class="panel-title">Episodes</h3>
        <form class="inline-form" @submit.prevent="createEpisode">
          <input v-model.number="newEpisode.number" type="number" min="1" placeholder="Episode #" required />
          <input v-model="newEpisode.airDate" type="date" />
          <button class="btn-pink" type="submit">Add Episode</button>
        </form>
        <div class="admin-list">
          <div v-for="ep in globalState.episodes" :key="ep.id" class="admin-row">
            <span>Episode {{ ep.episodeNumber }}</span>
            <span class="pill" :class="ep.status === 'published' ? 'pill-active' : 'pill-bombshell'">{{ ep.status }}</span>
            <button v-if="ep.status !== 'published'" class="btn-sm" @click="publish(ep.id)">Publish</button>
          </div>
        </div>
      </section>
      ...
      <section v-if="tab === 'points'" class="tab-panel">
        ...
        <form class="log-form" @submit.prevent="logPoints">
          <select v-model="logForm.episodeId" required>
            <option value="">— Episode —</option>
            <option v-for="ep in globalState.episodes" :key="ep.id" :value="ep.id">
      ...
      <!-- Scoring rules -->
      <section v-if="tab === 'rules'" class="tab-panel">
        <h3 class="panel-title">Scoring Categories</h3>
        <p class="panel-sub">Global rules inherited by every league.</p>
        <div class="rules-grid">
          <div v-for="c in globalState.scoringCategories" :key="c.id" class="rule-card">
            <span class="rule-pts" :class="{ neg: c.points < 0 }">{{ c.points > 0 ? '+' : '' }}{{ c.points }}</span>
            <span>{{ c.name }}</span>
            <span class="pill pill-active">{{ c.type }}</span>
          </div>
        </div>
      </section>
...
const newEpisode = reactive({ number: 1, airDate: '' })
const logForm = reactive({ episodeId: '', islanderId: '', categoryId: '', description: '', points: 0 })
...
async function createEpisode() {
  await addEpisode({ episodeNumber: newEpisode.number, airDate: newEpisode.airDate })
  newEpisode.number++
}

async function publish(id) {
  await publishEpisode(id)
}
...
.rule-pts { font-weight: 900; color: var(--pink); min-width: 36px; }
.rule-pts.neg { color: #c62828; }
